            ...
            if(requestStatus[m_clientFd].requestMethod == "POST"){
                ...
                if(requestStatus[m_clientFd].msgHeader["Content-Type"] == "multipart/form-data"){  // 如果发送的是文件
                    ...
                    if(requestStatus[m_clientFd].fileMsgStatus == FILE_CONTENT){
                        // 首先以二进制追加的方式打开文件
                        std::ofstream ofs("DataBase/" + requestStatus[m_clientFd].recvFileName, std::ios::out | std::ios::app | std::ios::binary);
                        if(!ofs){
...
void HandleSend::process(){
    ...
    if(responseStatus[m_clientFd].status == HANDLE_INIT){
        ...
        if(opera == "/"){                   //如果是根目录，返回文件夹中的所有文件名字
            ...
            // 先创建响应体对应的数据
            // 函数中先获取 /DataBase文件夹中的所有文件，然后根据 filelist.html 的页面结构，所有文件项加入页面，最终的HTML页面以字符串形式保存到 msgBody 中
            ...
            // 先创建响应体对应的数据
            // 获取所传递文件的描述符
            responseStatus[m_clientFd].fileMsgFd = open(("DataBase/" + filename).c_str(), O_RDONLY);
            ...
            // 在本地删除文件
            int ret = remove(("DataBase/" + filename).c_str());
            if(ret != 0){
...
void HandleSend::getFileListPage(std::string &fileListHtml){
    ...
    getFileVec("DataBase", fileVec);
    
    // 构建页面
    std::ifstream fileListStream("fileServer.html", std::ios::in);
    std::string tempLine;
    ...
    for(const auto &filename : fileVec){
        fileListHtml += "            <tr><td class=\"col1\">" + filename +
                    ...
                    "\">Download</a></td> <td class=\"col3\"><a href=\"delete/" + filename +
